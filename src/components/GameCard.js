      ...
      <div className="card" style={{ display: 'flex', flexDirection: 'column', height: '100%', padding: '20px' }}>
        ...
        {(guideUrl || logUrl) && (
          <div className="card__footer" style={{ borderTop: '1px solid var(--ifm-color-emphasis-200)', marginTop: 'auto', paddingTop: '16px', display: 'flex', gap: '8px', flexWrap: 'wrap', justifyContent: 'center' }}>
            {guideUrl && (
              <Link to={guideUrl} className="button button--secondary button--sm sublink-btn" style={{ flexGrow: 1, minWidth: '40%', fontSize: '0.9rem', borderWidth: '1px' }}>
                📖 가이드
              </Link>
            )}
            {logUrl && (
              <Link to={logUrl} className="button button--secondary button--sm sublink-btn" style={{ flexGrow: 1, minWidth: '40%', fontSize: '0.9rem', borderWidth: '1px' }}>
                📜 플레이 기록
              </Link>
            )}
          </div>
