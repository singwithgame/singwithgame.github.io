...
1. **가벼운 마크다운 기반 관리 (Low Code)**
   - 자바스크립트나 리액트 지식이 없어도, 깃허브 웹사이트에서 `.md` 파일만 작성하면 사이드바, 가이드, 세션 기록이 모두 자동으로 생성 및 업데이트됩니다. (단, 홈페이지 수정 시에는 복붙용 스니펫 활용)
2. **허브 홈페이지 (Home)**
...
- 🏠 **`src/pages/index.mdx`**: 홈페이지 화면입니다. 메인 타이틀(<h1>)을 수정하거나, 주어진 `<GameCard>` 스니펫을 복사-붙여넣기하여 홈 화면에 새로운 게임 링크를 띄울 수 있습니다. (이 파일은 예외적으로 `.mdx`로 작성되어 HTML/JSX 태그가 포함되어 있으나, 기존 코드를 참고하여 텍스트만 수정하시면 됩니다.)
