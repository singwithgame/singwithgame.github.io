...
1. **순수 텍스트 기반 관리 (No Code Required)**
   - 자바스크립트나 리액트 지식이 없어도, 깃허브 웹사이트에서 `.md` 또는 `.yml` 파일만 작성하면 홈페이지, 사이드바, 가이드, 세션 기록이 모두 자동으로 생성 및 업데이트됩니다.
2. **허브 홈페이지 (Home)**
...
- 🏠 **`config/games.yml`**: 홈페이지 화면에 표시될 **게임 카드** 목록을 설정하는 파일입니다.
...
### 1. 홈페이지에 게임 카드 추가하기 (`config/games.yml`)
...
### 3. 세션 기록(Log) 작성하기 (`botc-logs/`, `avalon-logs/` 등 내부)
